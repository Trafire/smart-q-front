...
function App() {
  ...
  async function nextQuestion() {
    ...
    let url = "http://10.0.0.102:8000/sentence/" + pk + "/"
    fetch(url, requestOptions)
      .then(response => response.json());

    const response = await fetch("http://10.0.0.102:8000/sentence/random-sentence/");
  ...
  async function confirm() {
    ...
    let url = "http://10.0.0.102:8000/sentence/" + pk + "/set-pose/"
    fetch(url, requestOptions)
      .then(response => response.json());
